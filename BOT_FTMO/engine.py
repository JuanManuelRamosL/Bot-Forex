...
def make_strategy(cfg):
    """Elige la estrategia según cfg.STRATEGY ('mean_reversion' por defecto, o 'smc')."""
    name = getattr(cfg, "STRATEGY", "mean_reversion")
    if name == "smc":
...
def quote_factor(instrument, price):
    """
    Convierte una ganancia expresada en la moneda de COTIZACIÓN a USD.
    - Pares XXX_USD (EUR_USD, GBP_USD, XAU_USD...): la cotización ya es USD -> factor 1.
    - Pares USD_XXX (USD_JPY, USD_CHF, USD_CAD...): la cotización es la otra divisa,
      hay que dividir por el precio para pasar a USD -> factor 1/price.
    Sin esto, el P&L de los pares USD-base queda mal escalado (ej. ~150x en USD_JPY).
    """
